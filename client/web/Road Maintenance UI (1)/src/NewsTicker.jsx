// NewsTicker.jsx — floating, semi-transparent news card w/ rotating items.
...
function NewsTicker({ items }) {
  ...
  const sevStyles = {
    closure: { dot: 'bg-signal-red',   label: 'CLOSURE', labelCls: 'text-signal-red' },
    alert:   { dot: 'bg-signal-amber', label: 'ALERT',   labelCls: 'text-signal-amber' },
    info:    { dot: 'bg-signal-green', label: 'UPDATE',  labelCls: 'text-signal-green' },
  };
  const s = sevStyles[current.severity] || sevStyles.info;

  return (
    <div className="absolute left-3 right-3 top-3 safe-top z-30 pointer-events-none">
      <div
        className="pointer-events-auto mx-auto max-w-md rounded-2xl bg-white/75 backdrop-blur-xl backdrop-saturate-150 ring-1 ring-black/5 shadow-lift overflow-hidden"
      ...
      >
        ...
        <div className="flex items-center gap-2 px-3.5 pt-3 pb-1.5">
          ...
          <span className={`text-[10px] font-bold tracking-[0.14em] ${s.labelCls} whitespace-nowrap`}>{s.label}</span>
          <span className="text-[10px] font-medium text-ink-400 tracking-wide whitespace-nowrap truncate">· LIVE FROM CITY OPS</span>
        ...
        {expanded && (
          <div className="border-t border-ink-100 bg-white/60 max-h-72 overflow-y-auto nice-scroll">
            {items.map((n, i) => {
              const ns = sevStyles[n.severity] || sevStyles.info;
              return (
                <button
                  key={n.id}
                  onClick={(e) => { e.stopPropagation(); setIdx(i); setExpanded(false); }}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`text-[10px] font-bold tracking-[0.14em] ${ns.labelCls}`}>{ns.label}</div>
                    <div className="text-[13.5px] font-semibold text-ink-900 leading-tight mt-0.5 truncate">{n.headline}</div>
